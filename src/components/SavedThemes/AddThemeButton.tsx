...
function AddThemeButton() {
  ...
  const handleSubmit = useCallback(
    (event: FormEvent<HTMLFormElement>) => {
      ...
      const reqBody = {name:addNewSavedTheme(event.target.themeName.value).savedTheme.name,...addNewSavedTheme(event.target.themeName.value).savedTheme.themeOptions}
      fetch('http://ec2-52-91-136-183.compute-1.amazonaws.com:4000/addTheme',{method: 'POST', body :JSON.stringify(reqBody),    headers: {
